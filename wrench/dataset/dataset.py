...
from .utils import (
    bag_of_words_extractor,
    tf_idf_extractor,
    sentence_transformer_extractor,
    bert_text_extractor,
    bert_relation_extractor,
...
class NumericDataset(BaseDataset):
    ...
    def extract_feature_(self, extract_fn: str, return_extractor: bool, **kwargs: Any):
        ...
        self.features = np.array(
            list(map(lambda x: x["feature"], self.examples)), dtype=np.float32
        )

        if return_extractor:
            return lambda y: np.array(
                list(map(lambda x: x["feature"], y)), dtype=np.float32
            )
    ...
    def extract_feature_(
        ...
        **kwargs: Any,
        ...
        if extract_fn == "bow":
            ...
            data, extractor = sentence_transformer_extractor(
                self.examples, device=device, model_name=model_name, **kwargs
            )
        elif extract_fn == "bert":
            data, extractor = bert_text_extractor(
                self.examples,
                device=device,
            ...
            raise NotImplementedError(
                f"feature extraction method {extract_fn} is not supported!"
            )
...
class RelationDataset(BaseDataset):
    ...
    def extract_feature_(
        ...
        **kwargs: Any,
        ...
        if extract_fn == "bert":
            data, extractor = bert_relation_extractor(
                self.examples,
                device=device,
                model_name=model_name,
                feature=feature,
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        super(ImageDataset, self).__init__(
            path=path, split=split, feature_cache_name=feature_cache_name, **kwargs
        )
    ...
    def extract_feature_(
        ...
        **kwargs: Any,
        ...
        if extract_fn == "pretrain":
            data, extractor = image_feature_extractor(
                self.examples, device=device, model_name=model_name, **kwargs
            )
        else:
            raise NotImplementedError(
