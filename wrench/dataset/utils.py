...
def check_weak_labels(dataset: Union[BaseDataset, np.ndarray]) -> np.ndarray:
    if isinstance(dataset, BaseDataset):
        assert dataset.weak_labels is not None, r"Input dataset has no weak labels!"
...
def split_labeled_unlabeled(
    dataset: BaseDataset, cut_tied: Optional[bool] = False
    ...
    for i, l in enumerate(weak_labels):
        ...
        if np.max(l) > -1:
            if not cut_tied or (
                len(lst) > 1 and lst[0][-1] - lst[1][-1] > 0
            ):  # or weaklabels_cnt<=1:
...
def split_conf_unconf_by_percentile(
    dataset: BaseDataset,
    ...
    assert percentile > 0 and percentile < 1, (
        f"percentile should be in (0, 1), now it's {percentile}!"
    )
    ...
    if return_thres and return_y:
        return (
            (conf_dataset, y[conf_idx]),
            (unconf_dataset, y[unconf_idx]),
            (max_probs.max(), max_probs.min(), thres),
        )
    elif return_thres:
...
def split_conf_unconf(
    dataset: BaseDataset,
    y: Optional[np.ndarray] = None,
    mode: Optional[str] = "thres",
    theta: float = 0.2,
    return_y=True,
    return_thres=False,
):
...
def bag_of_words_extractor(data: List[Dict], **kwargs: Any):
    ...
    extractor.count_vec = count_vect
...
def sentence_transformer_extractor(
    data: List[Dict],
    model_name: Optional[str] = "paraphrase-distilroberta-base-v1",
    **kwargs: Any,
):
    corpus = list(map(lambda x: x["text"], data))
    ...
    **kwargs: Any,
    ...
    def extractor(data: List[Dict]):
        ...
        for sentence in tqdm(corpus):
            ...
            if feature == "cls":
                output = (
                    model(inputs).pooler_output.cpu().squeeze().numpy()
                )  # [len(sentence), dim_out]
                text_features.append(output)
            elif feature == "avr":
                output = (
                    model(inputs).last_hidden_state.cpu().squeeze().numpy()
                )  # [len(sentence), dim_out]
...
def bert_relation_extractor(
    ...
    **kwargs: Any,
    ...
    @torch.no_grad()
    def extractor(data: List[Dict]):
        assert ("span1" in data[0]) and ("span2" in data[0]), (
            "Passed data missing span index."
        )
        assert ("entity1" in data[0]) and ("entity2" in data[0]), (
            "Passed data missing entity name."
        )
        corpus = list(map(lambda x: x["text"], data))
        span1_list, span2_list = (
            list(map(lambda x: x["span1"], data)),
            list(map(lambda x: x["span2"], data)),
        )  # char level
        ent1_list, ent2_list = (
            list(map(lambda x: x["entity1"], data)),
            list(map(lambda x: x["entity2"], data)),
        )
        model = AutoModel.from_pretrained(model_name, **kwargs).to(device)
        model.eval()
        tokenizer = AutoTokenizer.from_pretrained(model_name)  # e.g. 'bert-base-cased'
        ...
        for i, sentence in tqdm(enumerate(corpus)):
            ...
            inputs = torch.tensor(
                tokenizer.convert_tokens_to_ids(tokens), device=device
            ).unsqueeze(0)
            output = (
                model(inputs).last_hidden_state.cpu().squeeze().numpy()
            )  # [len(sentence), dim_out]
            cls_emb = output[0, :]
            ent1_emb, ent2_emb = (
                np.average(output[e1s:e1n, :], axis=0),
                np.average(output[e2s:e2n, :], axis=0),
            )
            if feature == "cat":
                text_features.append(np.concatenate([cls_emb, ent1_emb, ent2_emb]))
            elif feature == "avr":
                text_features.append(
                    np.average(np.concatenate([cls_emb, ent1_emb, ent2_emb]), axis=0)
                )
            else:
                raise NotImplementedError
        return np.array(text_features)
...
def create_unbalanced_set(data: BaseDataset, imbalance_ratio: int):
    ...
    imbalance_list = np.array(
        [int(n * miu**i) for (i, n) in enumerate(prior)]
    )  # n_i * μ^i
    ...
    sampled_ids = np.concatenate(
        [
            np.random.choice(ids[prior_cumsum[i] : prior_cumsum[i + 1]], n)
