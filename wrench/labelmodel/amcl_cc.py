...
class AMCL_CC(BaseLabelModel):
    def __init__(self, logger, seed: Optional[int] = None, **kwargs: Any):
        super().__init__()
        self.seed = seed
        ...
        **kwargs: Any,
        ...
        if self.n_samples > 0 and num_items_valid > self.n_samples:
            choices = np.random.choice(
                np.arange(num_items_valid), self.n_samples, replace=False
            )
        ...
        model_theta = self.sub_gradient_method2(
            L_train_aug,
            Y,
            constraints,
            self.brier_loss_linear_vectorized,
            self.linear_combination_labeler_vectorized,
            self.project_to_simplex,
            init_theta,
            T,
            h,
            num_label_fns,
    ...
    def compute_constraints_with_loss2(
        self,
        lf1,
        lf2,
        output_labelers_unlabeled,
        output_labelers_labeled,
        true_labels,
        train_labels=None,
    ):
        """
        ...
        self.logger.info(
            "Num WL: %d, Num Unlab %d, Num Classes %d, Num Lab %d" % (N, M, C, Ml)
        )
        ...
        for i in range(N):
            ...
            offset = (
                B
                * scaling_factor
                * np.sqrt((Ml + M) * np.log(4 * N / delta) / (2 * (Ml * M)))
            )
            ...
            if offset != 0:
                constraints.append(
                    cp.sum(cp.multiply(Y, build_coefficients)) <= cons_ub
                )
                constraints.append(
                    cp.sum(cp.multiply(Y, build_coefficients)) >= cons_lb
                )
            else:
                constraints.append(
                    cp.sum(cp.multiply(Y, build_coefficients)) == cons_lb
